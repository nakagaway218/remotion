...
def save_settings(source, destination, exclude_heavy_dirs):
...
def build_robocopy_command(source, destination, dry_run, exclude_heavy_dirs):
    command = [
        "robocopy",
        source,
        destination,
    ...
    if exclude_heavy_dirs:
        command.extend(["/XD", *DEFAULT_EXCLUDED_DIRS])
    if dry_run:
        command.append("/L")
    return command
...
class BackupSyncApp:
    def __init__(self, root):
        ...
        self.exclude_heavy_dirs_var = tk.BooleanVar(
            value=settings.get("exclude_heavy_dirs", True)
        )
    ...
    def build_ui(self):
        ...
        exclude_check = ttk.Checkbutton(
            main,
            text="重い開発用フォルダを除外する（.git / node_modules など）",
            variable=self.exclude_heavy_dirs_var,
        )
        exclude_check.pack(anchor="w", pady=(4, 8))
        ...
        self.write_output(
            ...
            "Desktop全体を同期する場合、GitHub配下の .git や node_modules は非常に重くなります。\n"
            "通常は「重い開発用フォルダを除外する」をオンのまま使ってください。\n\n"
        )
    ...
    def start_sync(self, dry_run):
        ...
        exclude_heavy_dirs = self.exclude_heavy_dirs_var.get()
        if not dry_run:
            answer = messagebox.askyesno(
                ...
                "重い開発用フォルダの除外: "
                f"{'オン' if exclude_heavy_dirs else 'オフ'}\n\n"
                "実行してよろしいですか？",
            )
            if not answer:
                return

        save_settings(source, destination, exclude_heavy_dirs)
        self.running = True
        ...
        thread = threading.Thread(
            ...
            args=(source, destination, dry_run, exclude_heavy_dirs),
            daemon=True,
        )
        thread.start()

    def run_robocopy(self, source, destination, dry_run, exclude_heavy_dirs):
