...
- The standard mode should create prompts that the user pastes into ChatGPT or Codex. NotebookLM is optional for extra source-document reading.
...
- `重要情報ソースリスト`, `Codex調査メモ`, `NotebookLMで作成した基礎知識メモ`, and `インポートした文献・資料` are optional, but when present they must be included in downstream prompts.
