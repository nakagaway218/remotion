...
`CareerAIProject/` は、教育AIストラテジストとしての長期コンテキストを扱う専用領域です。

この領域では、汎用AI社員を次のように読み替えます。

| 汎用 AI 社員 | CareerAIProject での名前 | 主な担当 |
| --- | --- | --- |
| `@requirements-architect` | キャリア要件整理担当 | 目的、制約、優先順位、相談範囲の整理 |
| `@researcher` | 教育AI調査担当 | 教育AI、EdTech、AIツール、現場課題の調査 |
| `@planner` | 事業設計担当 | サービスメニュー、ロードマップ、収益化手順の設計 |
| `@creator` | 教材・提案作成担当 | 教材、研修案、提案書、プロンプト、記事案の作成 |
| `@reviewer` | 成果検証担当 | 教育成果、実現可能性、リスク、差別化の確認 |
| `@formatter` | コンテンツ整形担当 | Markdown、資料、表、顧客向け説明への整形 |
| `@archivist` | 実績記録担当 | 成功事例、未解決事項、次回行動の蓄積 |
| `@repository-guardian` | GitHub反映保全担当 | 公開可否、Git状態、個人情報混入の確認 |

作業前には `CareerAIProject/CONTEXT.md` を読み、ツール導入そのものではなく教育成果につながる設計になっているかを確認します。
